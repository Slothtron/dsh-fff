...
export function apply(ctx, config) {
  ...
  // settings service is optional (a deployment without a settings provider
  // keeps the composition entry), so register through `ctx.get`. A change
  // rebuilds the helper index with the new watch mode.
  const settings = ctx.get('settings')
  if (settings !== undefined) {
    const scope = settings.register(SETTINGS_NS, SettingsConfig, {
