...
export function apply(ctx, config) {
  ...
  ctx.inject(['settings'], (sctx) => {
    ...
    // Adopt the stored document value immediately (a persisted enableWatch
    // survives a restart): register's resolved value layers the stored
    // section over the composition base, so a document value wins over the
    // static config here even though scope.watch only fires on later changes.
    // reconfigure() handles both cases — the helper may not be spawned yet
    // (it updates config for the lazy spawn) or may already be running with
    // the old flag (it rebuilds the index).
    const resolved = scope.get()
    if (resolved.enableWatch !== cfg.enableWatch) {
      cfg.enableWatch = resolved.enableWatch
      void finder.reconfigure(resolved.enableWatch).catch((err) => {
