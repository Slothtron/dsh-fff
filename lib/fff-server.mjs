#!/usr/bin/env node
/**
 * fff-server — resident FFF index process for the dsh-fff plugin.
 *
 ...
 * writes JSON responses on stdout (one per line). The parent dsh-fff plugin
 * spawns this process and proxies model-visible tool calls to it.
 *
 * Protocol:
...
function ensureFinder() {
  ...
  const init = FileFinder.create({
    basePath: baseRoot,
    aiMode: true,
    disableWatch: true, // one-shot search tools; no background watcher needed
  })
  if (!init.ok) {
