...
export class FffFinder {
  ...
  async spawn() {
    ...
    const argv = [nodeBin, this.config.serverPath, '--timeout', String(this.config.scanTimeoutMs)]
    // Pass the initial watch mode so a fresh helper starts in the configured
    // state (the settings card can flip it later via `reconfigure`).
    if (this.config.enableWatch === true) argv.push('--watch')
    const spec = {
      argv,
      cwd: process.cwd(),
      stdio: {
  ...
  /**
   * Flip the helper's watcher mode live. The helper destroys its current index
   * and rebuilds with the new flag on the next search; the session workspace
   * tracking is reset so the next call reindexes to its workspace.
   */
  async reconfigure(enableWatch) {
    this.config.enableWatch = enableWatch
    this.currentBase = null
    if (this.handle === null) return { ok: true, value: { disableWatch: !enableWatch } }
    const res = await this.call('reconfigure', { disableWatch: !enableWatch })
    if (!res.ok) throw new Error(`dsh-fff: reconfigure failed: ${res.error}`)
    return res
  }
